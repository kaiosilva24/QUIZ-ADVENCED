...
async function getDB() {
    ...
    try {
        ...
        // Retorna um objeto mockado (sem a classe Pool nativa que pode estar quebrada) para a aplicaÃ§Ã£o nÃ£o explodir
        poolInstance = {
            all: async () => [],
            get: async () => null,
            run: async () => ({ changes: 0 })
...
async function runMigrations(db) {
    ...
    await db.query(`
        CREATE TABLE IF NOT EXISTS quiz_events (
            ...
            step_id TEXT, -- ID do nÃ³ em que estÃ¡
            answer_value TEXT, -- OpÃ§Ã£o que escolheu
            created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
        );
    `);
