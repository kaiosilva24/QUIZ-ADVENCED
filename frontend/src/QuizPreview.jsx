  ...
  const resolveNextStep = (explicitId, currentButtonScoreTarget = null) => {
    ...
    if (isCurrentVariant) {
      if (explicitId) {
        ...
        if (!t && allSteps.length === 1) return explicitId; // Safamente confia no ID (SSR fast)
      }
      while (nextIdx < allSteps.length && allSteps[nextIdx].isVariant) nextIdx++;
      if (nextIdx < allSteps.length) return allSteps[nextIdx].id;
      if (allSteps.length === 1) return '__NEXT__'; // Avança cegamente (SSR fast)
      return null;
    }
               ...
               return (
                 ...
                 >
                    <div style={{ width: '100%', aspectRatio: '1/1', background: 'rgba(255,255,255,0.05)', overflow: 'hidden' }}>
                      {opt.imageSrc ? <img src={opt.imageSrc} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : null}
                    </div>
